import { Injectable } from '@angular/core';

@Injectable()
export class SerialNumberDataService {
  serialNumberData: object;

  fetch_company(company) { return this.serialNumberData[company]}

  fetch_countries_for_a_company(company) { return Object.keys(this.serialNumberData[company]) }
  
  fetch_companies() { return Object.keys(this.serialNumberData) }

  constructor() { 
    this.serialNumberData = {
      squier: {
        china: {
          minimumPrefixSize: 3,
          maximumPrefixSize: 6,
          'ca5': '1995', 'ca6': '1996', 'ca7': '1997', 'ca8': '1998', 'ca9': '1999', 'cae95': '1995', 'cae96': '1996',
          'cae97': '1997', 'cae98': '1998', 'cae99': '1999', 'cae00': '2000', 'cae01': '2001', 'cae02': '2002',
          'cae03': '2003', 'cae04': '2004', 'cae05': '2005', 'cae06': '2006', 'cae07': '2007', 'cae08': '2008',
          'cae09': '2009', 'cae10': '2010', 'cae11': '2011', 'cd5': '1995', 'cd6': '1996', 'cd7': '1997', 'cd8': '1998',
          'cd9': '1999', 'cgrl95': '1995', 'cgrl96': '1996', 'cgrl97': '1997', 'cgrl98': '1998', 'cgrl99': '1999',
          'cgrl00': '2000', 'cgrl01': '2001', 'cgrl02': '2002', 'cgrl03': '2003', 'cgrl04': '2004', 'cgrl05': '2005',
          'cgrl06': '2006', 'cgrl07': '2007', 'cgrl08': '2008', 'cgrl09': '2009', 'cgrl10': '2010', 'cgrl11': '2011',
          'cgs95': '1995', 'cgs96': '1996', 'cgs97': '1997', 'cgs98': '1998', 'cgs99': '1999', 'cgs00': '2000',
          'cgs01': '2001', 'cgs02': '2002', 'cgs03': '2003', 'cgs04': '2004', 'cgs05': '2005', 'cgs06': '2006',
          'cgs07': '2007', 'cgs08': '2008', 'cgs09': '2009', 'cgs10': '2010', 'cgs11': '2011', 'cj5': '1995',
          'cj6': '1996', 'cj7': '1997', 'cj8': '1998', 'cj9': '1999', 'co5': '1995', 'co6': '1996', 'co7': '1997',
          'co8': '1998', 'co9': '1999', 'cob95': '1995', 'cob96': '1996', 'cob97': '1997', 'cob98': '1998',
          'cob99': '1999', 'cob00': '2000', 'cob01': '2001', 'cob02': '2002', 'cob03': '2003', 'cob04': '2004',
          'cob05': '2005', 'cob06': '2006', 'cob07': '2007', 'cob08': '2008', 'cob09': '2009', 'cob10': '2010',
          'cob11': '2011', 'cos95': '1995', 'cos96': '1996', 'cos97': '1997', 'cos98': '1998', 'cos99': '1999',
          'cos00': '2000', 'cos01': '2001', 'cos02': '2002', 'cos03': '2003', 'cos04': '2004', 'cos05': '2005',
          'cos06': '2006', 'cos07': '2007', 'cos08': '2008', 'cos09': '2009', 'cos10': '2010', 'cos11': '2011',
          'ct5': '1995', 'ct6': '1996', 'ct7': '1997', 'ct8': '1998', 'ct9': '1999', 'cxs95': '1995', 'cxs96': '1996',
          'cxs97': '1997', 'cxs98': '1998', 'cxs99': '1999', 'cxs00': '2000', 'cxs01': '2001', 'cxs02': '2002',
          'cxs03': '2003', 'cxs04': '2004', 'cxs05': '2005', 'cxs06': '2006', 'cxs07': '2007', 'cxs08': '2008',
          'cxs09': '2009', 'cxs10': '2010', 'cxs11': '2011', 'cy1': '2001', 'cy2': '2002', 'cy3': '2003', 'cy4': '2004',
          'cy5': '2005', 'cy6': '2006', 'cy7': '2007', 'cy8': '2008', 'cy95': '1995', 'cy96': '1996', 'cy97': '1997',
          'cy98': '1998', 'cy99': '1999', 'cy00': '2000', 'cy01': '2001', 'cy02': '2002', 'cy03': '2003', 'cy04': '2004',
          'cy05': '2005', 'cy06': '2006', 'cy07': '2007', 'cy08': '2008', 'cy09': '2009', 'cy10': '2010', 'cy11': '2011',
          'nc5': '1995', 'nc6': '1996', 'nc7': '1997', 'nc8': '1998', 'nc9': '1999', 'yn5': '1995', 'yn6': '1996',
          'yn7': '1997', 'yn8': '1998', 'yn9': '1999'
        },
        india: {
          minimumPrefixSize: 4,
          maximumPrefixSize: 6,
          'cs07': '2007',
          'nhs10': '2010',
          'nhs11': '2011',
          'nhsa09': '2009',
          'nhsd09': '2009',
          'nhse09': '2009',
          'nhsf09': '2009',
          'nhsg09': '2009',
          'nhsh09': '2009',
          'nhsi09': '2009',
          'nhsj08': '2008',
          'nhsj09': '2009',
          'nshk09': '2009',
          'sh07': '2007',
          'sh08': '2008',
          'zssh07': '2007',
          'zssh08': '2008'
        },
        indonesia: {
          minimumPrefixSize: 4,
          maximumPrefixSize: 5,
          'ic96': '1996', 'ic97': '1997', 'ic98': '1998', 'ic99': '1999', 'ic00': '2000', 'ic01': '2001', 'ic02': '2002',
          'ic03': '2003', 'ic04': '2004', 'ic05': '2005', 'ic06': '2006', 'ic07': '2007', 'ic08': '2008', 'ic09': '2009',
          'ic10': '2010', 'ic11': '2011', 'ic12': '2012', 'ic13': '2013', 'ic14': '2014', 'ic15': '2015', 'ic16': '2016',
          'ic17': '2017', 'ics96': '1996', 'ics97': '1997', 'ics98': '1998', 'ics99': '1999', 'ics00': '2000',
          'ics01': '2001', 'ics02': '2002', 'ics03': '2003', 'ics04': '2004', 'ics05': '2005', 'ics06': '2006',
          'ics07': '2007', 'ics08': '2008', 'ics09': '2009', 'ics10': '2010', 'ics11': '2011', 'ics12': '2012',
          'ics13': '2013', 'ics14': '2014', 'ics15': '2015', 'ics16': '2016', 'ics17': '2017', 'is96': '1996',
          'is97': '1997', 'is98': '1998', 'is99': '1999', 'is00': '2000', 'is01': '2001', 'is02': '2002', 'is03': '2003',
          'is04': '2004', 'is05': '2005', 'is06': '2006', 'is07': '2007', 'is08': '2008', 'is09': '2009', 'is10': '2010',
          'is11': '2011', 'is12': '2012', 'is13': '2013', 'is14': '2014', 'is15': '2015', 'is16': '2016', 'is17': '2017',
          'si96': '1996', 'si97': '1997', 'si98': '1998', 'si99': '1999', 'si00': '2000', 'si01': '2001', 'si02': '2002',
          'si03': '2003', 'si04': '2004', 'si05': '2005', 'si06': '2006', 'si07': '2007', 'si08': '2008', 'si09': '2009',
          'si10': '2010', 'si11': '2011', 'si12': '2012', 'si13': '2013', 'si14': '2014', 'si15': '2015', 'si16': '2016',
          'si17': '2017'
        },
        japan: {
          minimumPrefixSize: 1,
          maximumPrefixSize: 2,
          'a': '(mij) 1985 - 1986 or (cij) 1997 - 1998',
          'b': '(mij) 1985 - 1986 or (cij) 1998 - 1999',
          'c': '1985 - 1986',
          'e': '1984 - 1987',
          'f': '1986 - 1987',
          'g': '1987 - 1988',
          'h': '1988 - 1989',
          'i': '1989 - 1990',
          'j': '1989 - 1990',
          'jv': '1982 - 1984',
          'k': '1990 - 1991',
          'l': '1991 - 1992',
          'm': '1992 - 1993',
          'n': '(mij) 1993 - 1994 or (cij) 1995 - 1996',
          'o': '1997 - 1998',
          'p': '(mij) 1993 - 1994 or (cij) 1999 - 2002',
          'q': '(mij) 1993 - 1994 or (cij) 2002 - 2004',
          'r': '2004 - 2006',
          's': '(mij) 1994 - 1995 or (cij) 2006 - 2008',
          'sq': '1983 - 1984',
          't': '(mij) 1994 - 1995 or (mij "post 2006") 2007 - 2010',
          'u': '(mij) 1995 - 1996 or (mij "post 2006") 2010 - 2011',
          'v': '1996 - 1997'
        },
        korea: {
          minimumPrefixSize: 2,
          maximumPrefixSize: 4,
          'cn0': '1990', 'cn1': '1991', 'cn2': '1992', 'cn3': '1993', 'cn4': '1994', 'cn5': '1995', 'cn6': '1996',
          'cn7': '1997', 'e7': '1987', 'e8': '1988', 'e9': '1989', 'e0': '1990', 'e1': '1991', 'e2': '1992', 'e3': '1993',
          'e4': '1994', 'kc97': '1997', 'kc98': '1998', 'kc99': '1999', 'kc00': '2000', 'kc01': '2001', 'kc02': '2002',
          'kc03': '2003', 'kc04': '2004', 'kc05': '2005', 'kc06': '2006', 'kc07': '2007', 'kc08': '2008', 'kc19': '2019',
          'kc10': '2010', 'kc11': '2011', 'kc12': '2012', 'kc13': '2013', 'kc14': '2014', 'kc15': '2015', 'kc16': '2016',
          'kc17': '2017', 'kv97': '1997', 'kv98': '1998', 'kv99': '1999', 'kv00': '2000', 'kv01': '2001', 'kv02': '2002',
          'kv03': '2003', 'kv04': '2004', 'kv05': '2005', 'kv06': '2006', 'kv07': '2007', 'kv08': '2008', 'kv19': '2019',
          'kv10': '2010', 'kv11': '2011', 'kv12': '2012', 'kv13': '2013', 'kv14': '2014', 'kv15': '2015', 'kv16': '2016',
          'kv17': '2017','m0': '1990', 'm1': '1991', 'm2': '1992', 'm3': '1993', 'm4': '1994', 'm5': '1995', 'm6': '1996',
          'm7': '1997', 'm8': '1998', 'm9': '1999', 'nk0': '1990', 'nk1': '1991', 'nk2': '1992', 'nk3': '1993',
          'nk4': '1994', 'nk5': '1995', 'nk6': '1996', 'nk7': '1997', 'nk8': '1998', 'nk9': '1999', 's9': '1989',
          's0': '1990', 's1': '1991', 's2': '1992', 's3': '1993', 's4': '1994', 'vn0': '1990', 'vn1': '1991',
          'vn2': '1992', 'vn3': '1993', 'vn4': '1994', 'vn5': '1995', 'vn6': '1996', 'vn7': '1997'
        },
        mexico: {
          minimumPrefixSize: 3,
          maximumPrefixSize: 3,
          'mn3': '1993',
          'mn4': '1994',
          'mn5': '1995',
          'mn6': '1996',
          'mn7': '1997',
          'mn8': '1998',
          'mn9': '1999'
        },
        'united states': {
          minimumPrefixSize: 1,
          maximumPrefixSize: 2,
          'e9': '1989',
          'n0': '1990',
          'n1': '1991',
          'n2': '1992',
          'n3': '1993',
          '0': '1991 - 1993'
        }
      }
    }

  }

}
